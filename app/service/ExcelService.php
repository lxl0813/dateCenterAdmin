    ...
    public function shares_details_export(array $shares_base, array $shares_more, $shares_cv)
    {
        ...
        try {
            ...
            foreach ($shares_base as $key => $item) { // 行写入
                $span = ord("A");
                unset($item['create_time']);unset($item['shares_id']);unset($item['create_by']);unset($item['create_account']);unset($item['create_admin']);
                foreach ($item as $keyName => $value) { // 列写入
                    if(chr($span)=="A"){
                        $curSheet->setCellValue(chr($span) . $column, $id);
                    }else{
...
//            ob_end_clean();//清除缓冲区,避免乱码
            ...
            if(empty($shares_more)){
                ...
                foreach ($shares_more as $key => $item) { // 行写入
                    $span = ord("A");
                    unset($item['shares_id']);unset($item['create_time']);unset($item['create_by']);unset($item['create_account']);unset($item['create_admin']);
                    foreach ($item as $keyName => $value) { // 列写入
                        if(chr($span)=="A"){
                            $curSheet->setCellValue(chr($span) . $column, $id);
                        }else{
