...
class FinanceController extends RbacController
{
    ...
    public function finance_account_balance(Request $request)
    {
        ...
        if ($company_id == "") {
            $this->resultError('公司ID不存在！');
            return;
        }
        if ($income_pay == "") {
            $this->resultError('收支金额为获取！');
            return;
        }
        if ($income_pay_type == "") {
            $this->resultError('收支类型为获取！');
            return;
        }
        if ($bank_id == "") {
            $this->resultError('银行账户未获取！');
            return;
        }
    ...
    public function finance_water_edit_do(Request $request)
    {
        ...
        $where['water_id'] = $param['water_id'];
        unset($param['water_id']);
    ...
    public function finance_account_balance_edit(Request $request)
    {
        ...
        if ($income_pay == "") {
            $this->resultError('收支金额未获取！');
            return;
        }
        if ($income_pay_type == "") {
            $this->resultError('收支类型未获取！');
            return;
        }
        if ($bank_id == "") {
            $this->resultError('银行账户未获取！');
    ...
    public function finance_attach_upload_do(Request $request)
    {
        ...
        try {
            ...
            if (!$find_res) {
                ...
                if ($insert_res) {
                    $this->resultSuccess('成功！');
                    return;
                } else {
                    unlink('.' . $path);
                    $this->resultError('失败1！');
                    return;
                }
            } else {
